...
use App\AllClass\flatRateContract\ChangeInchargeOfFixedRateContacts\AllChangeInchargeOfFixedRateContract; 
use App\AllClass\flatRateContract\ChangeInchargeOfFixedRateContacts\ChangeInchargeOfFixedRateContract;
...
class ChangeInchargeOfFixedRateContractController extends Controller
{
    public $headers = [
        '担当SE' => 'formatted_contractor',
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $validator = ChangeInchargeOfFixedRateContract::validate($request, $bango);
        ...
        $headers = ChangeInchargeOfFixedRateContract::headers($bango);
        ...
        $table_headers = ChangeInchargeOfFixedRateContract::headers($bango, 'table_headers');
        $page_no = ChangeInchargeOfFixedRateContract::$pageNo;
        $route = 'changeInchargeOfFixedRateContractTableSetting';
        $redirect_path = 'changeInchargeOfFixedRateContractReload';
        $temp_table = 'all_change_incharge_of_fixed_rate_contract_temp';
        ...
        if (isset($data_from_view['Button']) && ($data_from_view['Button'] == 'Thesearch' || $data_from_view['Button'] == 'FirstSearch' || $data_from_view['Button'] == 'sort' || $data_from_view['Button'] == 'xls' || $data_from_view['Button'] == 'refresh')) {
            ...
            try {
                if ($data_from_view['Button'] == 'Thesearch' || $data_from_view['Button'] == 'FirstSearch' || $data_from_view['Button'] == 'sort' || $data_from_view['Button'] == 'xls' || $data_from_view['Button'] == 'refresh') {
                    ...
                    //    $pagi = 20;
                    //    return view('flatRateContract.changeInchargeOfFixedRateContracts.mainChangeInchargeOfFixedRateContracts', compact('bango', 'tantousya', 'B9Data_left', 'C1Data_left', 'C2Data_left', 'B9Data_right', 'C1Data_right', 'C2Data_right', 'incharges', 'personal_datatxt0003', 'personal_datatxt0004', 'personal_datatxt0005', 'data_from_view', 'buttonMessage', 'headers', 'table_headers', 'redirect_path', 'page_no', 'route', 'pagi'));
                    //}
                    
                    if(($data_from_view['Button'] == 'Thesearch' || $data_from_view['Button'] == 'sort' || $data_from_view['Button'] == 'refresh') && $defalut_check == 0){
                        $pagi = 20;
                        return view('flatRateContract.changeInchargeOfFixedRateContracts.mainChangeInchargeOfFixedRateContracts', compact('bango', 'tantousya', 'pagi', 'incharges','data_from_view', 'buttonMessage',  'headers', 'table_headers', 'redirect_path', 'page_no', 'route'));
                    }

                    $query = AllChangeInchargeOfFixedRateContract::data($bango, $req_data)->toSql();
                    //dd($query,$data);
                    $allChangeInchargeOfFixedRateContract = $this->searchDataFetch($query, $data, $bango, $temp_table, $pagination);
                    if ($allChangeInchargeOfFixedRateContract->items() == null && $allChangeInchargeOfFixedRateContract->currentPage() != 1) {
                        $currentPage = ($allChangeInchargeOfFixedRateContract->lastPage());
                   ...
                   // dd($allChangeInchargeOfFixedRateContract);
                    if ($allChangeInchargeOfFixedRateContract->total() == 0) {
                        $exceedChangeInchargeOfFixedRateContract = '該当するデータがありません。';
                    } else {
                        $exceedChangeInchargeOfFixedRateContract = '';
                    }
                    ...
                    if ($data_from_view['Button'] == 'xls') {
                        ...
                        $excelName = '定期定額契約担当変更.xlsx';
                        return $this->excelDownload($headers, $searched, $excelName);
                    }
                }
           } catch (\Exception $e) {
               $exceedChangeInchargeOfFixedRateContract = '検索形式が間違っています。';
               $allChangeInchargeOfFixedRateContract = [];
               $allChangeInchargeOfFixedRateContract = collect($allChangeInchargeOfFixedRateContract);
               $allChangeInchargeOfFixedRateContract = $allChangeInchargeOfFixedRateContract->paginate($pagination);

               return view('flatRateContract.changeInchargeOfFixedRateContracts.mainChangeInchargeOfFixedRateContracts', compact('bango', 'tantousya', 'incharges', 'data_from_view', 'buttonMessage',  'headers', 'table_headers', 'redirect_path', 'page_no', 'route', 'fsReqData', 'allChangeInchargeOfFixedRateContract', 'exceedChangeInchargeOfFixedRateContract'));;
           }
 
            return view('flatRateContract.changeInchargeOfFixedRateContracts.mainChangeInchargeOfFixedRateContracts', compact('bango', 'tantousya', 'incharges', 'data_from_view', 'buttonMessage',  'headers', 'table_headers', 'redirect_path', 'page_no', 'route', 'req_data', 'fsReqData', 'allChangeInchargeOfFixedRateContract', 'exceedChangeInchargeOfFixedRateContract'));
        }

        $pagi = !empty($data_from_view['pagination']) ? $data_from_view['pagination'] : 20;
        //        if (request('change_id')) {
        //            $query = AllChangeInchargeOfFixedRateContract::data($bango, $deleted_item)->whereRaw("bango = '" . request('change_id') . "'")->toSql();
        //        } else {
        //            $query = AlChangeInchargeOfFixedRateContract::data($bango, $deleted_item)->toSql();
        //        }
    ...
    public function tableSetting($id, $user_default = null)
    {
        ...
        $initial_header = kengen::where('kengenchar01', 'col')->where('kengenchar03', $id)->where('kengenchar05', '0305')->get()->count();
        $pageNo = ChangeInchargeOfFixedRateContract::$pageNo;
    ...
    public function tableSettingSave(Request $request, $id, $type = null)
    {
        ...
        $pageNo = ChangeInchargeOfFixedRateContract::$pageNo;
        TableSetting::settingSave($request, $id, $pageNo, $this->headers, '定期定額契約担当変更', $type);
    }
    ...
    public function updateChangeInchargeOfFixedRateContract(Request $request){
            ...
            foreach($support_numbers as $key=>$val){
                ...
                if($prev != $update){
                    ...
                    $orderhenkan_update_data = [
                       ...
                       // dd($datachar07,$update);
                   $orderhenkanUpdate = QueryHelper::updateData('orderhenkan', $orderhenkan_update_data, ['datachar07' => $datachar07], $bango, __CLASS__, __FUNCTION__, __LINE__);
    ...
    public function checkChangeInchargeOfFixedRateContractUpdateData(Request $request){
