...
use App\AllClass\other\grossProfitAdjustmentInput\OrderDetails;
use App\AllClass\other\grossProfitAdjustmentInput\GrossProfitAdjustmentValidation;
use App\AllClass\other\grossProfitAdjustmentInput\GrossProfitAdjustmentInputDataEntry;
use App\AllClass\order\orderEntry\OrderEntry;

class GrossProfitAdjustmentInputController extends Controller
{
    public function index(){
        ...
        $name = QueryHelper::select(['bango','name'])->from('tantousya')->where("DeleteFlag = '0' ")->where("mail5 != '' ")->where("ztanka = '$ztanka' ")->orderBy("bango asc")->get()->execute();
        ...
        return view('other.grossProfitAdjustmentInput.mainGrossProfitAdjustmentInput',compact('bango', 'tantousya', 'name', 'categorykanriesU1','request1s', 'U2Data', 'c4Categorykanries', 'data309', 'data310', 'data311'));
    }
    ...
    public function orderDetails(Request $request, $bango)
    {
        ...
        $query = OrderDetails::data($bango, $orderId);
        $orderDetail = QueryHelper::fetchSingleResult($query);
        $ordertypebango2 = QueryHelper::fetchSingleResult("select max(ordertypebango2) from orderhenkan where kokyakuorderbango = '$orderId' and datachar10 is not null ")->max ?? 0;
        $salesDetail = QueryHelper::fetchSingleResult("Select * from orderhenkan where synchroorderbango = '0' and kokyakuorderbango = '$orderId' and datachar10 is not null and ordertypebango2 = $ordertypebango2");
        // dd($orderDetail);
        $errorMessage = array();
        ...
        if(empty($orderDetail)){
            array_push($errorMessage, "該当するデータがありません（". $orderId ."）");
        }
        if($orderDetail && $orderDetail->datachar04 == '2'){
            array_push($errorMessage, "売上済でありません。受注訂正を実施してください。");
        }
    ...
    public function generateCategoryWiseTable(Request $request)
    {
        ...
        $querystring .= " and  syouhin1.isuriage = 0 order by syouhin1.kokyakusyouhinbango limit 30 ";
