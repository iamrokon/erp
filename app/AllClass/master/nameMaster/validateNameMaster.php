...
use App\AllClass\CheckDuplicateCategory2;
use App\AllClass\CheckCategory2CharCount;
use App\AllClass\master\nameMaster\checkValidation;

class validateNameMaster
{
    public static function validate($request, $bango)
    {
        ...
        if (Input::has('field')) {
            ...
            $rules['category1'] = ['required', 'regex:/^([a-zA-Z0-9])*$/', 'max:2', new HasUnderAACheck];
            $rules['category2'] = ['required', 'max:16', new specialCharValidation, new CheckDuplicateCategory2($request['category1'], $request['category2'])];
            $rules['category3'] = ['required', 'max:20',new zenkaku, new specialCharValidation];
            $rules['category4'] = ['required',  'max:20',new zenkaku,new specialCharValidation];
            $rules['category5'] = ['required', new specialCharValidation, 'max:10']; // 'required',
            $rules['groupbango'] = ['required', new zenkaku, new specialCharValidation, 'max:10']; // 'required',
            $rules['osusume'] = ['required', 'numeric', 'max:99', new specialCharValidation,new CheckCategory2CharCount($request['category2'])];
            $rules['suchi1'] = ['required', 'max:999', 'numeric', new specialCharValidation]; // 'required',
        ...
        $message['email'] = '【:attribute】 入力形式が間違っています。';
