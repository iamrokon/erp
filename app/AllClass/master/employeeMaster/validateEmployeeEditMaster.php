...
class validateEmployeeEditMaster
{
    public static function validate($request, $bango)
    {
        ...
        if (Input::has('field')) {
            ...
            $rules['mail2'] = ['nullable', 'regex:/^([0-9])*$/', 'max:11'];
            $rules['mail3'] = ['nullable', 'regex:/^([0-9])*$/', 'max:11'];
            $rules['mail'] = ['required', 'email', 'max:70', new specialCharValidation, 'confirmed','unique:tantousya,mail,' . $request['bango'] . ',bango'];
...
//            $rules['inpemp2'] = ['bail', 'nullable', new specialCharValidation, 'max:60', 'string', new imageFormatValidator, 'unique:tantousya,datatxt0029,' . $request['bango'] . ',bango'];
//            $rules['inpemp4'] = ['bail', 'nullable', new specialCharValidation, 'max:60', 'string', new imageFormatValidator2, 'unique:tantousya,syounin,' . $request['bango'] . ',bango'];
            $rules['inpemp2'] = ['bail', 'nullable', new specialCharValidation, 'max:60', 'string', new imageFormatValidator];
            $rules['inpemp4'] = ['bail', 'nullable', new specialCharValidation, 'max:60', 'string', new imageFormatValidator2];

            $rules['innerlevel'] = ['required', 'regex:/^([0-9])*$/', 'numeric', 'between:10,20', new specialCharValidation, function ($attribute, $value, $fail) use ($innerLevel) {
                if ($value < $innerLevel) {
...
//        $message['datatxt0029.mimes'] = '【:attribute】jpeg  jpg  gif  png  bmp  pdf のみOK。';
        $message['inpemp2.regex'] = '【:attribute】半角英数字および"-"以外は使用できません。';
        $message['inpemp4.regex'] = '【:attribute】半角英数字および"-"以外は使用できません。';
        ...
        $attributes = [
            ...
            'syozoku' => '事業所CD',
            'passwd' => 'パスワード',
            'passwd_confirmation' => '(確認用)',
            'mail4' => '権限CD',
            'mail2' => '電話番号',
            'mail3' => '内線番号',
            'mail' => 'メールアドレス',
            'mail_confirmation' => '(確認用)',
            'datatxt0030' => '入力者1',
            'datatxt0031' => '入力者2',
            'datatxt0032' => '入力者3',
            'datatxt0033' => '入力者4',
