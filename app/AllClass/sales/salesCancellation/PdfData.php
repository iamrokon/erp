...
namespace App\AllClass\sales\salesCancellation;
...
Class PdfData{
    public static function data($bango,$deleted_item=2,$kokyakuorderbango=null,$kaiinid=null){

        QueryHelper::runQuery("DROP TABLE IF EXISTS pdf_data_temp");

        QueryHelper::runQuery(
        "CREATE TEMPORARY TABLE pdf_data_temp as
        select 
        orderhenkan.bango,
        ...
        CASE
            WHEN (substring(tuhanorder.information3,1,6) != substring(tuhanorder.information1,1,6)) AND (substring(tuhanorder.information3,1,6) != substring(tuhanorder.information2,1,6)) THEN concat(substring(kokyaku1Information3.name,1,30),' ','様分')
        ...
        substring_byte(syukkoold.syouhinname,1,60) as syouhinname,
        CASE
        ...
        where syukkoold.hantei = 0
        and syouhinid = '$kokyakuorderbango'
        and kaiinid = '$kaiinid'

        ORDER BY syouhinsyu ASC");

        
        $data=DB::table('pdf_data_temp');
