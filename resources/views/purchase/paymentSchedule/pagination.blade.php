@php
    if(isset($paymentScheduleInfos)){
        $current_page=$paymentScheduleInfos->currentPage();
        $per_page=$paymentScheduleInfos->perPage();
        $first_data= ($current_page - 1)*$per_page+1;
        $last_data=($current_page - 1)*$per_page+ sizeof($paymentScheduleInfos->items());
        $total=$paymentScheduleInfos->total();
        $lastPage=$paymentScheduleInfos->lastPage();
    }else{
        $current_page = 1;
        $per_page = 20;
        $first_data = 1;
    ...
    <div class="col-7">
        <div class="pagi-content mt-3">
            <table>
                <tbody>
                <tr>
                    @if(isset($paymentScheduleInfos) && $paymentScheduleInfos->lastPage() > 1)
                        @include('layout.pagination_new.pagination_new1')
                    @endif

                    @include('layout.pagination_new.pagination_new2')

                    @if(isset($paymentScheduleInfos) && $paymentScheduleInfos->total() > 0)
