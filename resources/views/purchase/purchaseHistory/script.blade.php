...
{{-- Header Click to Check/Uncheck Checkbox  --}}
<!--<script type="text/javascript">
  // 指示
...
{{--number input preventing special charecter input--}}
<script>
    document.querySelector("#purchaseNoFrom").addEventListener("keypress", function (evt) {
        if (evt.which != 8 && evt.which != 0 && evt.which < 48 || evt.which > 57)
        {
            evt.preventDefault();
        }
    });
    document.querySelector("#purchaseNoTo").addEventListener("keypress", function (evt) {
        if (evt.which != 8 && evt.which != 0 && evt.which < 48 || evt.which > 57)
        {
            evt.preventDefault();
        }
