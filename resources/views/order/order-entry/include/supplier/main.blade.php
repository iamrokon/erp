    ...
    <div class="modal-dialog" role="document" style="max-width: 1106px!important;">
        <div class="modal-content" data-bind="nextFieldOnEnter:true">
                                ...
                                $('#office_search_button').click(function () {
                                    ...
                                    if(searchValue == "" || !pattern.test(searchValue)){
                                        ...
                                        if($("#"+companyid).length>0){
                                            ...
                                            //reset business partner information
                                            resetBusinessPartnerInfo();
                                            $("#supplier_err_msg").html("検索結果に該当するデータがありません1");
                                        } 
                                    }
                            ...
                            <div class=" page4_table_design mt-2  table_hover  table-head-only">
                                ...
                                <div id="personal_master_content_div">
                                    ...
                                    <div style="height: 85px; background: #fff;">
                                        <div id="personal_master_content_div_table" class=" modal-table-white"
            ...
            <div class="modal-footer pl-4 pr-4">
                ...
                <input id="fillable_id_input_status" type="hidden"/>

                <button type="button" id="clear_parent" class="btn text-white w-145 bg-teal mr-2">親画面をクリア</button>
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for (var i = 0; i < response.length; i++) {
                    ...
                    html += '<tr class="show_content_last trfocus" tabindex="0" data-serial="'+response[i]['datatxt0049']+'"  id="ets_' + response[i]['bango'] + '">'
                    /* for (var key in response[i])
                       {*/
                    //console.log(key+'='+ response[i][key]);
                ...
                for (var i = 0; i < response.length; i++) {
                    ...
                    //console.log(response);
                    document.getElementById("ets_"+response[i]['bango']).setAttribute("onclick", 'goToEtsuransyaDetail(' + d + ',"' + b + '","' + c + '")');
                }
    ...
    function sum() {
        ...
        var fillable_id = $(document).find("#fillable_id").val();
        var db_fillable_id = $(document).find("#db_fillable_id").val();
        var fillableValue = kokyakuAddr.toString() + "/" + haisouHaism.toString() + "/" + etsuransyaMail4.toString();
        var dbHiddenValue = kokyaku + haisou + etsuransya;
        document.getElementById(fillable_id).value = fillableValue
        ...
        if(current_class.includes("deliveryDestination")){
            localStorage.setItem("deliveryDestination", fillableValue+"hidden"+ dbHiddenValue)
