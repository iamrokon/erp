...
<form id="mainForm" action="{{ route('changeInchargeOfInHouseWorkWithFixedRateContract') }}" method="post">
        ...
        <div class="row">
          <div class="col">
            <div class="wrapper-pagination" style="background-color:#fff;height:130px;padding: 10px;">
               ...
               @include('flatRateContract.changeInchargeOfInHouseWorkWithFixedRateContracts.pagination')
               <!----------pagination End----------------->
              <div class="row">
                  <div class="col-6">
                  </div>
            ...
            <div style="overflow: hidden;">
              <div class="table-responsive largeTable">
               ...
               @if(isset($allChangeInChargeOfInHouseWorkWithFixedRateContract))
                   @foreach($allChangeInChargeOfInHouseWorkWithFixedRateContract as $key=>$val)
                    <tr>
                      @foreach($headers as $header=>$field)  
                           ...
                           @elseif($field == 'formatted_new_charge')
                               ...
                               <div class="custom-arrow">
                                   <select class="form-control new_charge" onchange="checkChangeInchargeOfInHouseWorkWithFixedRateContractUpdateData($(this),'{{$bango}}')"  name="selected_new_charge[]">
                                          <option value="1">-</option>
                                              @foreach ($incharges as $user)
                                                  @php
                                                      $data = $user->bango .' ' . substr($user->name,0,3);
                                      ...
                                      <input type="hidden" class="up_support_number" name="up_support_number[]" id="" value="{{$val->contract_number}}">                                    
                                      <input type="hidden" class="prev_new_charge" name="prev_new_charge[]" id="" value="{{ $val->formatted_new_charge}}">
                              </td>      
                             @else
                                  <td>{{$val->$field}}</td>
                             @endif
