...
function updateChangeInchargeOfFixedRateContract(url) {
    buttonPress++;
    if (buttonPress == 1) {
        var count = 0;
            ...
            $.ajax({
                ...
                success:function(result){
                    if($.trim(result) == 'ok'){
                        ...
                        $('#error_data').html("");
                        var error_message = "登録はまだ完了していません。内容をご確認後、もう１度登録ボタンを押してください。"
                        ...
                        html += '</div>';
                        $(document).find("#confirmation_message").html(html);
                        buttonPress = 0;
                    }
                    // else{
...
function checkChangeInchargeOfFixedRateContractUpdateData(own,login_bango){
    console.log(own.val());
    if(own.val() != 1){
        ...
        $.ajax({
            type:'get',
            url:'change-incharge-of-fixed-rate-contract/checkChangeInchargeOfFixedRateContractUpdateData/'+login_bango,
