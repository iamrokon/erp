...
$(document).ready(function () {
    ...
    $(document).on('change', "select[name='orderAmountClassification[]']",function (e) {
        ...
        if (category == 'V130' || category == 'V140'){
            ...
            $element.find('.responsiblePerson').val('').change();
            $element.find('.responsiblePerson').prop('disabled', false);
            $element.find('.responsiblePersonCD').val('');
        }
    })
    domBody.on("click","#registrationButton", function (e) {   
        ...
        if (button == 1){
            $.ajax({
                ...
                type: "POST",
                url: "purchase-completion-cancellation/register/" + bango,
                data: data,
                processData: false,
                contentType: false,
...
function readSourceOrderDetails(){
    ...
    if (orderId.length >= 10) {    
        $.ajax({
            ...
            type: "POST",
            url: "purchase-record-transfer/source_order_details/" + bango,
            data: {
                orderId
            },
            success: function (response) {
                ...
                if(response.hasOrder != 0){
                    if($("#order_number_101").hasClass("error")){
                        $("#error_data").empty();
                        $("#order_number_101").removeClass("error");
                    }
                    var orderDetails = response.orderDetails;
                    console.log(orderDetails);
                    if(orderDetails['data_102']){
                        $('#data_102').val(orderDetails['data_102']);
                    }
                    if(orderDetails['data_103']){
                        $('#data_103').val(orderDetails['data_103']);
                    }
                }else{
                        $("#data_102").val('');
                        $("#data_103").val('');
                        var html = '<div style="margin-top: 8px;margin-left:-8px!important;"> <p>' + errorMessage + '</p> </div>';
                        $('#error_data').html(html);
                        $("#error_data").show();
                        $("#order_number_101").addClass("error")
...
function readDestinationOrderDetails(){
    var orderId = $(this).val();
    var bango = $("input[id='userId']").val();
    if (orderId.length >= 10) {    
        $.ajax({
            ...
            type: "POST",
            url: "purchase-record-transfer/destination_order_details/" + bango,
            data: {
                orderId
            },
            success: function (response) {
                ...
                var errorMessage = response.errorMessage;            
                if(response.hasOrder != 0){
                    if($("#order_number_301").hasClass("error")){
                        $("#error_data").empty();
                        $("#order_number_301").removeClass("error");
                    }
                    var orderDetails = response.orderDetails;
                    console.log(orderDetails);
                    if(orderDetails['data_302']){
                        $('#data_302').val(orderDetails['data_302']);
                    }
                    if(orderDetails['data_303']){
                        $('#data_303').val(orderDetails['data_303']);
                    }
                }else{
                        $("#data_302").val('');
                        $("#data_303").val('');
                        ...
                        $('#error_data').html(html);
                        $("#error_data").show();
                        $("#order_number_301").addClass("error")
                        // $('#registrationButton').prop('disabled', true);               
                }
            }
        })
...
$(document).on("keyup", "#order_number_101",readSourceOrderDetails);
$(document).on("keyup", "#order_number_301",readDestinationOrderDetails);
